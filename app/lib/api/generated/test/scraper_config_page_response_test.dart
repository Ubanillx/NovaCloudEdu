import 'package:test/test.dart';
import 'package:nova_api/nova_api.dart';

// tests for ScraperConfigPageResponse
void main() {
  final instance = ScraperConfigPageResponseBuilder();
  // TODO add properties to the builder and call build()

  group(ScraperConfigPageResponse, () {
    // 配置列表
    // BuiltList<ScraperConfigResponse> records
    test('to test the property `records`', () async {
      // TODO
    });

    // 总记录数
    // int total
    test('to test the property `total`', () async {
      // TODO
    });

    // 当前页码
    // int pageNum
    test('to test the property `pageNum`', () async {
      // TODO
    });

    // 每页大小
    // int pageSize
    test('to test the property `pageSize`', () async {
      // TODO
    });

    // 总页数
    // int totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });
  });
}
