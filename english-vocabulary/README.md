...
本目录提供了将词库数据导入 PostgreSQL 数据库的工具脚本，用于智云星课「每日单词」功能：

| 脚本 | 功能 |
|:---|:---|
| `convert_json_to_pgsql.py` | 将 `json-full` 目录下的 JSON 文件转换为 PostgreSQL INSERT 语句，导入 `daily_word` 表 |
| `fetch_audio_urls.py` | 抓取单词发音 URL（支持有道词典/金山词霸/Google TTS），生成带音频的 SQL 导入脚本 |
| `daily_word_import_with_audio.sql` | 上述脚本生成的最终 SQL 文件（含发音 URL） |

### 使用方法

```bash
# 1. 生成基础导入 SQL
python3 convert_json_to_pgsql.py

# 2. 抓取发音 URL 并生成带音频的 SQL
python3 fetch_audio_urls.py

# 3. 导入数据库
psql -U postgres -d NovaCloudEdu -f daily_word_import_with_audio.sql
```

### 乱序 SQL

`乱序sql/` 目录下提供了各级别的乱序词库 SQL 导入脚本（初中~SAT 共 7 级别）。
