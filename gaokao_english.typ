...
// ==================== 数据读取 ====================
#let paper-title = if "title" in data { data.title } else { "英语试卷" }
...
#set text(
  font: ("Noto Serif CJK SC", "Noto Sans CJK SC", "Times New Roman"),
  size: font-size,
  lang: "zh",
)
...
#columns(2, gutter: 1.2cm)[
  ...
  #align(center)[
    ...
    #text(size: 10.5pt, fill: rgb("#333333"))[
      英语 #h(1.5em) 考试时间：#str(duration-min) 分钟 #h(1.5em) 满分：#str(total-score) 分
    ]
  ]
    ...
    for q in section.questions {
      ...
      if "options" in q and q.options != none and q.options.len() > 0 {
        ...
        if opt-count <= 4 {
          ...
          let cols = if max-len > 20 { 2 } else { calc.min(opt-count, 4) }
          grid(
            columns: (1fr,) * cols,
            row-gutter: 0.4em,
            column-gutter: 0.5em,
      ...
      // 不同题型的作答空间（英语：完形填空/阅读选择紧凑，写作留白大）
      if "type" in q {
