...
import type { GroupResponse, MessageReadUserResponse } from '../../api/generated/models';
...
const GroupChatWindow: React.FC<GroupChatWindowProps> = ({
  groupId, groupName, groupAvatar, onBack,
}) => {
  ...
  const handleUploadSend = useCallback((content: string, type: string) => {
    ...
    sendGroupMessage(groupId, content, type);
  ...
  const handleSend = () => {
    ...
    try {
      sendGroupMessage(groupId, content, 'TEXT');
    } catch {
      toast.error('发送失败');
    } finally {
      setSending(false);
      ...
      >
        ...
        {loading ? (
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.messageId} className={`flex gap-2 ${self ? 'flex-row-reverse' : ''}`}>
