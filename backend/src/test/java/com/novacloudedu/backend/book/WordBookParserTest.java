...
import com.novacloudedu.backend.infrastructure.parser.WordBookParser;
...
@DisplayName("Word文档解析器测试")
class WordBookParserTest {

    private WordBookParser parser;
    private String testFilePath;

    @BeforeEach
    void setUp() {
        parser = new WordBookParser();
        String projectRoot = System.getProperty("user.dir");
        testFilePath = Paths.get(projectRoot, "test-flie", "智云星课文档.docx").toString();
    }
    ...
    void testSupports() {
        assertTrue(parser.supports(FileType.DOCX));
    ...
    @Test
    @Order(2)
    @DisplayName("解析Word文件 - 成功")
    void parseWordFile_Success() {
    ...
    @Test
    @Order(3)
    @DisplayName("解析Word文件 - 章节索引正确")
    void parseWordFile_ChapterIndexCorrect() {
        File testFile = new File(testFilePath);
    ...
    @Test
    @Order(4)
    @DisplayName("解析Word文件 - 内容包含HTML标签")
    void parseWordFile_ContentContainsHtml() {
        File testFile = new File(testFilePath);
