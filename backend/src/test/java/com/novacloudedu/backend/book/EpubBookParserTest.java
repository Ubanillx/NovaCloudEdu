...
import com.novacloudedu.backend.infrastructure.parser.EpubBookParser;
...
class EpubBookParserTest {
    ...
    void testSupports() {
        assertTrue(parser.supports(FileType.EPUB));
    ...
    @Test
    @Order(2)
    @DisplayName("解析EPUB文件 - 成功")
    void parseEpubFile_Success() {
    ...
    @Test
    @Order(3)
    @DisplayName("解析EPUB文件 - 章节索引正确")
    void parseEpubFile_ChapterIndexCorrect() {
        File testFile = new File(testFilePath);
    ...
    @Test
    @Order(4)
    @DisplayName("解析EPUB文件 - 内容包含HTML标签")
    void parseEpubFile_ContentContainsHtml() {
        File testFile = new File(testFilePath);
