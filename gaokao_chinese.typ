...
// ==================== 数据读取 ====================
#let paper-title = if "title" in data { data.title } else { "语文试卷" }
#let paper-subtitle = if "subtitle" in data { data.subtitle } else { none }
#let duration-min = if "duration_min" in data { data.duration_min } else { 150 }
...
#set par(leading: 0.85em, justify: true, first-line-indent: 2em)
...
#columns(2, gutter: 1.2cm)[
  ...
  #align(center)[
    ...
    #text(size: 10.5pt, fill: rgb("#333333"))[
      语文 #h(1.5em) 考试时间：#str(duration-min) 分钟 #h(1.5em) 满分：#str(total-score) 分
    ]
  ]
  ...
  #block(width: 100%, inset: (x: 0.5cm, y: 0.4cm), radius: 0pt,
    ...
    5. 本试卷包含现代文阅读、古代诗文阅读、语言文字运用和写作四个部分。
  ]
